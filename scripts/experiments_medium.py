...
def main():
    ...
    # data paths (dev dataset generated by scripts/02_sample_dev.py)
    base = Path(__file__).resolve().parents[1] / "mimiciv_backdoor_study" / "data"
    dev_dir = base / "dev"
    parquet_path = dev_dir / "dev.parquet"
    splits_json = base / "splits" / "splits.json"

    if not parquet_path.exists():
        raise FileNotFoundError(f"Dev parquet not found at {parquet_path}; run scripts/02_sample_dev.py first")
